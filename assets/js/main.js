...
const titles={dashboard:["Centro de operación","Visibilidad ejecutiva, cargas y cuellos de botella"],orders:["Control de pedidos","Consulta, trazabilidad y operación integral"],sales:["Registro de ventas","Creación de pedidos y control comercial"],credit:["Solicitudes de crédito","Radicación, estudio y decisión"],cartera:["Cartera","Validación de riesgo y liberación"],caja:["Caja","Pagos, soportes y liberación financiera"],purchasing:["Compras","Abastecimiento y órdenes PVE"],receiving:["Recepción","Ingreso documental, físico, calidad y stickers"],picking:["Alistamiento","Picking, checklist y novedades"],cutting:["Corte","Tomas de tiempo, chipas y desperdicio"],billing:["Facturación","Factura, soporte y liberación"],shipping:["Despachos y entregas","Rutas locales, nacionales, recogidas y cierre"],inventory:["Inventario","Existencias, lotes, ubicaciones y movimientos"],approvals:["Aprobaciones","Excepciones, cancelaciones, cambios y reaperturas"],vsm:["VSM y tiempos","Lead time, valor, espera y productividad"],reports:["Reportes","Indicadores operativos y exportaciones"],imports:["Importar histórico","Carga controlada de pedidos cerrados por CSV"],qa:["Bot QA E2E","192 combinaciones comerciales y 10 controles empresariales"],audit:["Auditoría","Registro inmutable de decisiones y movimientos"],admin:["Administración","Usuarios, roles, calendarios y reglas"]};

async function bootAuthenticated(){
  document.querySelector("#app").innerHTML=loading("Validando sesión, roles y configuración…");
  try{
    ...
    initRouter(async route=>{
      ...
      const moduleId=route.module;const [title,sub]=titles[moduleId]||["ERP Supply Enterprise",""];updateShell(moduleId,title,sub);
