...
export async function renderCutting(root){
  ...
  root.innerHTML=`
    <section class="page-head cutting-page-head">
      <div><span class="cutting-kicker">Centro de corte</span><h2>Corte por referencia</h2><p>Selecciona una referencia y completa la secuencia operativa: inicio, origen, ejecución y cierre con evidencia.</p></div>
...
async function openCutGroup(groupKey,{message=""}={}){
  ...
  try{
    ...
    host.innerHTML=`<div class="modal-overlay"><section class="modal"><header class="modal-head"><h3>No fue posible abrir la referencia</h3><button class="icon-btn" data-close aria-label="Cerrar">×</button></header><div class="modal-body"><p class="danger">${fmt.escape(error.message)}</p></div></section></div>`;
    bindClose(host);
  }
}
...
function renderPaused(host,data){
  ...
  host.innerHTML=`<div class="modal-overlay cutting-overlay"><section class="modal cutting-guided-modal cut-paused-modal"><header class="modal-head cut-guide-head"><div><span class="cut-guide-kicker">CORTE PAUSADO · ${fmt.escape(g.reference||e.reference||"")}</span><h3>${fmt.escape(g.description||e.description||"Material por cortar")}</h3><p>El tiempo productivo está detenido hasta que indiques que vas a continuar.</p></div><button class="icon-btn" data-close aria-label="Cerrar">×</button></header><div class="modal-body cut-guide-body"><section class="cut-state-screen paused"><span class="cut-state-mark">Ⅱ</span><div><small>PAUSA REGISTRADA</small><h4>Continúa solo cuando realmente vuelvas a cortar</h4><p>${fmt.escape(p.reason||"Pausa operativa")}</p></div><section class="cut-state-facts"><div><small>Inicio del corte</small><strong>${dateTime(e.startedAt)}</strong></div><div><small>Estado</small><strong>Tiempo activo detenido</strong></div></section><button class="btn btn-primary cut-main-cta" data-resume-cut>Continuar corte</button></section></div>${guidedFooter("Cerrar el popup no reanuda el corte. Debes usar “Continuar corte”.")}</section></div>`;
  bindClose(host);
