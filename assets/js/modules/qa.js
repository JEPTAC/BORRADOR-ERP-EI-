  ...
  root.innerHTML=`
    <section class="qa-total-page">
      <header class="qa-total-hero card">
        ...
        <div class="qa-total-seal"><span>QA</span><strong>10.25.2</strong><small>Release Certification</small></div>
      </header>
...
async function detail(id,type){
  if(type==="TOTAL_ROBOT"){
    ...
    const certificateBlock=cert?`<div class="section-gap-small"></div><section class="qa-release-certificate"><div class="card-head compact"><div><span class="qa-total-eyebrow">CERTIFICADO DE LIBERACIÓN</span><h3 class="${cert.certified?"success":"danger"}">${cert.certified?"ERP CERTIFICADO":"ERP NO CERTIFICADO"}</h3><p>Estado: ${fmt.escape(cert.releaseState||"—")} · versión ${fmt.escape(cert.version||"10.25.2")}</p></div></div><div class="qa-summary qa-release-gates">${gate("Rutas canónicas",gates.routing,g=>`${g.passed||0}/336 aprobadas · ${g.pending||0} pendientes`)}${gate("Recorridos completos",gates.journeys,g=>`${g.passed||0}/336 aprobados · ${g.pending||0} pendientes`)}${gate("Campaña EXTREME",gates.extreme,g=>`${g.executed||0}/${g.planned||0} ejecutados · ${g.failed||0} fallidos · timeout ${g.timeouts||0} · transporte ${g.transport||0}`)}${gate("Interfaz",gates.interface,g=>`${g.passedModules||0}/${g.expectedModules||0} módulos`)}${gate("Integridad",gates.integrity,g=>`${g.passed||0}/${g.expected||0} checks`)}${gate("Responsive",gates.responsive,g=>`${g.passed||0}/${g.expected||0} viewports/módulos`)}${gate("Acciones Sandbox UI",gates.sandboxUi,g=>`${g.passed||0}/${g.expected||0} acciones`)}${gate("Limpieza",gates.cleanup,g=>`${g.remainingTestOrders||0} pedidos TEST remanentes`)}</div></section>`:"";
