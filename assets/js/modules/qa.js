...
import {runTotalQaRobot,runDeepQaCampaign,robotCheckRows} from "./qa-total.js";
...
export async function renderQa(root){
  ...
  root.innerHTML=`
    <section class="qa-total-page">
      <header class="qa-total-hero card">
        ...
        <div class="qa-total-seal"><span>QA</span><strong>10.25.1</strong><small>Deep + Capacity</small></div>
      </header>
      ...
      <section class="qa-total-grid">
        ...
        <aside class="qa-total-scope card">
          ...
          <div class="qa-scope-item"><strong>288</strong><div><b>Ciclos transversales</b><span>Cada tipo y etapa: nota, novedad, reporte, espera, aprobación y cancelación.</span></div></div>
  ...
  await Promise.all([load(root),loadCapacity(root)]);
}
...
async function detail(id,type){
  if(type==="TOTAL_ROBOT"){
    const data=await api.qaRobotDetail(id),run=data.run||{},checks=data.checks||[],deep=data.deepSummary||{},deepFailures=data.deepFailures||[];
