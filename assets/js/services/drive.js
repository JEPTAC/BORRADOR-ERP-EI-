...
/**
 * V10.23 · Evidencias de Mi Jornada.
 * Mantiene compatibilidad con el puente institucional existente enviando
 * executionId también como orderId; los despliegues nuevos de Apps Script
 * utilizan contextType/contextId para organizar la carpeta como ACTIVIDAD.
 */
export async function uploadWorkEvidence(executionId,file,evidenceType="FINAL_PHOTO",title="Actividad"){
  if(!executionId)throw new Error("No se recibió la actividad asociada a la evidencia.");
  ...
  const shortId=String(executionId).slice(0,8).toUpperCase();
  const uploaded=await submitToBridge({
    uploadId,
    origin:window.location.origin,
    accessToken:session.access_token,
    // Compatibilidad hacia atrás con el Apps Script V10.22.x.
    orderId:String(executionId),
    orderNumber:`ACT-${shortId}`,
    contextType:"ACTIVITY",
    contextId:String(executionId),
    contextLabel:safeName(title||`ACT-${shortId}`,`ACT-${shortId}`),
    category:`WORK_${String(evidenceType||"FINAL_PHOTO").toUpperCase()}`,
  ...
  const record=await api.workRegisterEvidence(executionId,{
    evidenceType:String(evidenceType||"FINAL_PHOTO").toUpperCase(),
    driveFileId:uploaded.id,
    fileName:uploaded.name,
    mimeType:uploaded.mimeType||file.type||"application/octet-stream",
    sizeBytes:Number(uploaded.size||file.size),
