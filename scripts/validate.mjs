...
check(read("assets/js/modules/qa.js").includes('finishLabel:"Iniciar certificación"')&&read("assets/js/modules/qa.js").includes("onFinish:()=>")&&!read("assets/js/modules/qa.js").includes('confirmLabel:"Iniciar certificación",onConfirm'),"El botón Certificar flujo completo no está conectado al callback onFinish del wizard.");
...
check(!read("assets/js/modules/qa-total.js").includes("if(transportFailures.length){break}"),"La campaña V10.25.2 todavía se detiene ante un fallo de transporte.");
check(read("assets/js/modules/qa-total.js").includes("found&&opened&&primary.executed")&&read("assets/js/modules/qa-total.js").includes("found&&opened&&startedCut"),"Los probes Sandbox V10.25.2 pueden aprobar sin abrir/ejecutar la acción primaria.");
check(!read("assets/js/modules/qa-total.js").includes("runQaV1022(true)"),"La prueba total V10.25.2 todavía llama la matriz monolítica susceptible a statement_timeout.");
check(read("assets/js/modules/qa-total.js").includes("ERP CERTIFICADO PARA LIBERACIÓN")&&read("assets/js/modules/qa.js").includes("CERTIFICADO DE LIBERACIÓN"),"La UI no distingue un certificado release de una ejecución QA parcial.");
check(sql.includes("qa_existing_order_id")&&sql.includes("failure_step_code")&&sql.includes("failure_action")&&sql.includes("error_sqlstate"),"V10.25.1 no conserva diagnóstico del error original o protección contra order_id revertido.");
check(sql.includes("qa_deep_cases")&&sql.includes("erp_x_qa_robot_build_deep_campaign")&&sql.includes("erp_x_qa_robot_execute_deep_case")&&sql.includes("NO_DELIVERY"),"V10.25.1 no contiene campaña profunda aislada o ramas de no-entrega/devolución.");
check(sql.includes("qa_capacity_runs")&&sql.includes("erp_x_qa_capacity_record")&&sql.includes("erp_x_qa_capacity_runs"),"V10.25.1 no conserva historial de capacidad/concurrencia.");
check(fs.existsSync(path.join(root,"tests/load/erp-capacity.js"))&&fs.existsSync(path.join(root,"scripts/record-k6-summary.mjs"))&&fs.existsSync(path.join(root,".github/workflows/qa-capacity.yml")),"Falta la suite k6 o su integración CI de capacidad.");
check(read("tests/load/erp-capacity.js").includes("BREAKPOINT")&&read("tests/load/erp-capacity.js").includes("SPIKE")&&read("tests/load/erp-capacity.js").includes("SOAK")&&read("tests/load/erp-capacity.js").includes("p(95)<1800"),"La suite de capacidad no contiene perfiles de carga, spike, soak y breakpoint con umbrales.");
check(executable.includes("runDeepQaCampaign")&&executable.includes("Pulso concurrente")&&executable.includes("Prueba exhaustiva cruzada"),"La interfaz QA V10.25.1 no expone campaña profunda o medición concurrente.");
