...
-- ============================================================
-- MIGRATION 026: sales address and simple shipping v10.11.6
-- ============================================================
begin;
...
-- V10.11.7 · Corrección de controles heredados
begin;

-- V10.11.7 · Corrección de controles heredados en Despachos y Cierre.
-- El flujo actual valida explícitamente:
--   Despacho: pedido tomado + guía registrada -> pasar a cierre.
--   Cierre: pedido tomado + foto de entrega -> pedido finalizado.
-- Los checklists antiguos de estas etapas eran invisibles en el nuevo popup y
-- bloqueaban COMPLETE aunque el usuario hubiera cumplido el microproceso actual.

-- 1) Retirar los controles heredados de las plantillas para tareas futuras.
update erp_supply.checklist_templates
set required = false,
    active = false
where step_code in (
...
and item_code in ('DELIVERY_EVIDENCE','DOCUMENTS_COMPLETE','DELIVERY_CONFIRMED');

-- 2) Liberar tareas abiertas que ya nacieron con esos controles antiguos.
-- No se marcan como "completados" para no falsear auditoría; simplemente dejan
-- de ser requisitos del flujo vigente.
update erp_supply.task_checklist tc
set required = false,
    metadata = coalesce(tc.metadata,'{}'::jsonb) || jsonb_build_object(
      'retiredBy','V10.11.7',
      'retiredReason','Control heredado sustituido por validación transaccional del microproceso de despacho/cierre',
      'retiredAt',now()
    )
from erp_supply.order_tasks t
where t.id = tc.task_id
  and t.step_code in ('CLIENT_POINT','CLIENT_PICKUP','LOCAL_DISPATCH','NATIONAL_DISPATCH','CLOSURE')
  and t.status in ('QUEUED','ASSIGNED','IN_PROGRESS','WAITING','BLOCKED')
  and tc.required = true;

-- 3) Alinear el gate transaccional con los microprocesos vigentes.
-- Se conserva toda la lógica más reciente de Facturación/PVP y solo se corrigen
-- las condiciones de Despachos y Cierre.
create or replace function erp_supply.validate_task_completion()
returns trigger
language plpgsql
security definer
    ...
    when 'FACTURACION' then
      if upper(v_order.order_type_code)='PVP' then
        ...
        if not exists(
          ...
          raise exception 'Debe cargar la factura antes de enviar el pedido a despacho';
        end if;
      end if;
...
-- El trigger ya existe, pero se recrea para garantizar que use la función actual.
drop trigger if exists trg_validate_task_completion on erp_supply.order_tasks;
create trigger trg_validate_task_completion
before update of status on erp_supply.order_tasks
for each row execute function erp_supply.validate_task_completion();
...
-- V10.12 · Flujos paralelos, excepciones y aprobaciones
-- ERP EI V10.12
