-- V10.33.0: REQUEST_APPROVAL valida destinos contra la matriz real de permisos.
...
begin
  ...
  if v_action='REQUEST_APPROVAL' then
    ...
    if not exists(
      ...
      join erp_supply.roles r on r.code=mp.role_code and r.active
      where mp.module_code='approvals'
        and mp.role_code=v_assigned
        and mp.can_approve
    ) then
      raise exception 'El rol seleccionado no tiene capacidad para decidir aprobaciones';
    end if;

    if v_type='PRIORITY' and upper(coalesce(p_payload->>'priority','')) not in('LOW','MEDIUM','HIGH','URGENT','CRITICAL') then
      raise exception 'Prioridad inválida';
    end if;
    if v_type='ROUTE_CHANGE' and not exists(
      select 1 from erp_supply.delivery_routes where code=p_payload->>'route' and active
    ) then
  ...
  return erp_supply.execute_action_internal(
    p_order_id,v_action,coalesce(p_payload,'{}'::jsonb),v_actor,false,p_expected_version,p_idempotency_key
  );
end;
$$;
